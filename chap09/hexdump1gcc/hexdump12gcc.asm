; Source file   : x64ASM_sbs4/chap09/hexdump1gcc/hexdump12gcc.asm
; Created       : Fri Aug  1 16:00:30 +01 2025
; Version       : 1.2
...
; * Output only the updated part of the HexStr. No more renmant from previous
...
Scan_for:
    ...
    ; write(1, &HexStr, <HexStr converted length + '\n'>)
    mov rax, 1                              ; sys_write ID
    mov rdi, 1                              ; arg_01: stdout file descriptor
    mov rsi, HexStr                         ; arg_02: destination pointer
    ; HACK: Using HEXLEN write unnecessary remnants characters from previous
    ; hexa conversion even though last sys_read input is much less than HEXLEN
    ; mov rdx, HEXLEN                         ; arg_03: write byte num
    ; This is the better way to populate sys_write arg#3:
    add rdx, 3                              ; Account for last iteration hexa
    mov byte [HexStr + rdx], 0ah            ; Append '\n'
    inc rdx                                 ; Account for '\n'
    syscall                                 ; invoke kernel
